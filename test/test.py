...
def parse_args(args):
    ...
    parser.add_argument('--vss_volume', default=False, action='store_true', help="Run tests for VSS volume enumeration filtering logic")
...
if __name__ == '__main__':
    ...
    if to_test.all_tests or to_test.small_canary:
        ...
        print(f"{result.testsRun} tests were run - {len(result.skipped)} tests skipped.")
        print(f"{len(result.errors)} Errors. {len(result.failures)} Failures")
    if to_test.all_tests or to_test.vss_volume:
        print("\nTesting VSS Volume Enumeration Filtering:")
        result = runner.run(test_unit.VssVolumeSuite())
        print(f"{result.testsRun} tests were run - {len(result.skipped)} tests skipped.")
        print(f"{len(result.errors)} Errors. {len(result.failures)} Failures")
